...
@IonicPage()
@Component({
	templateUrl: "action-sheet-02.html"
})
export class ActionSheet02 {
	result: string = "";
			...
			buttons: [
				{
					...
					handler: () => {
						...
						alert.addButton({
							...
							handler: () => {
								this.result =
									"pressed Cancel button in alert from action sheet";
							}
						});
						alert.addButton({
							text: "Okay",
							handler: () => {
								this.result =
									"pressed Okay button in alert from action sheet";
							}
						});
